from logger import logger
from task_1 import task_1_menu
from task_2 import task_2_menu
from task_3 import task_3_menu

"""
Главный модуль программы.
Обеспечивает интерфейс верхнего уровня и доступ к отдельным заданиям.
"""   

def main():
    """
    Запускает главное меню программы.

    Пользователь может выбрать одно из трёх заданий или завершить работу.

    Меню включает:
        1. Задание 1 — поворот матрицы на 90 градусов.
        2. Задание 2 — выполнение сложения и вычитания больших чисел,
           представленных массивами цифр.
        3. Задание 3 — поиск общих чисел двух массивов, включая реверсированные.
        4. Выход из программы.

    Функция работает в цикле, пока пользователь явно не завершит программу.
    Взаимодействует с пользователем через консоль.
    Все действия пользователя логируются с помощью ``logger``.

    task3_menu : Меню задания 1 (Подсчет общих чисел).
    task4_menu : Меню задания 2 (Поиск индексов при условии).
    task8_menu : Меню задания 3 (Сложение и сортировка массивов).

    Пример:
    >>> main()
    ГЛАВНОЕ МЕНЮ
    1. Задание 1 (Подсчет общих чисел)
    2. Задание 2 (Поиск индексов при условии)
    3. Задание 3 (Сложение и сортировка массивов)
    4. Выход
    Выберите пункт: 1
    # ... дальнейшая работа программы
    """

    while True:
        print("\nГЛАВНОЕ МЕНЮ")
        print("1. Задание 1 (Подсчет общих чисел)")
        print("2. Задание 2 (Поиск индексов при условии)")
        print("3. Задание 3 (Сложение и сортировка массивов)")
        print("4. Выход")

        choice = input("Выберите пункт: ")
        logger.info(f"Пользователь выбрал главный пункт меню: {choice}")

        if choice == "1":
            task_1_menu()
        elif choice == "2":
            task_2_menu()
        elif choice == "3":
            task_3_menu()
        elif choice == "4":
            print("Выход...")
            break
        else:
            print("Неверный пункт!")
            logger.info("Ошибка: неверный пункт главного меню")


if __name__ == "__main__":
    main()